package com.food_service_application.data.repository
...
import com.food_service_application.data.remote.FoodApiService
...
class FoodRepository @Inject constructor(
    ...
    suspend fun addToCartFromDetail(food: Food, quantity: Int): Resource<String> = withContext(
        Dispatchers.IO) {
        try {
            when (val cartResult = getCartFoods()) {
                is Resource.Success -> {
    ...
    suspend fun updateCartQuantity(cartFood: CartFood, newQuantity: Int): Resource<String> = withContext(
        Dispatchers.IO) {
        try {
            if (newQuantity <= 0) {
